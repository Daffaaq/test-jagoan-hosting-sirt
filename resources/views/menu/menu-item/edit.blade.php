                ...
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item">
                        <a href="{{ route('menu-item.index') }}"
                            class="{{ request()->routeIs('menu-item.index') ? 'active' : '' }}">Menu Group Management</a>
                    </li>
                    <li class="breadcrumb-item">
                        <a href="{{ route('menu-item.create') }}"
                            class="{{ request()->routeIs('menu-item.create') ? 'active' : '' }}">Create Menu Group</a>
                    </li>
                </ol>
            </div>
            <div class="card-body">
                <!-- Menu Group Create Form -->
                <form method="POST" action="{{ route('menu-item.update', $menuItem->id) }}">
                    @csrf
                    @method('PUT')
                    <!-- Name -->
